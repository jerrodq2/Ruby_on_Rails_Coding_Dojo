RSpec.configure do |config|
  ...
  def create_shoe(user, product='created shoe', date=Time.new().to_date, amount=40)
    User.find(user.id).shoes.create(product:product, date:date, amount: amount)
  end

  def create_purchase(buyer, shoe, seller, date=Time.new().to_date)
    Purchase.create(date: date, buyer: buyer, seller: seller, shoe: shoe)
  end
